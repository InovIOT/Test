...
st.image("images/logo_main.png", use_column_width=True)

page_config()
styling()

# Initialize translations dictionary at the module level
translations = {
    "Svenska": {
...
prompt = current_translations["chat_prompt"]
...
with col1:
    if st.button(current_translations["chat_clear_chat"], type="secondary"):
        if "messages" in st.session_state.keys():
            st.session_state.messages = [
                {"role": "assistant", "content": current_translations["chat_hello"]}
            ]

with col2:
    with st.expander(current_translations["chat_settings"]):
        llm_model = st.selectbox(
            current_translations["chat_choose_llm"], 
        ...
        llm_temp = st.slider(
            current_translations["chat_choose_temp"],
            min_value = 0.0,
            max_value = 1.0,
            step = 0.1,
        ...
        with st.form("my_form"):
            prompt_input = st.text_area(current_translations["chat_system_prompt"], prompt, height=200)
            st.session_state.system_prompt = prompt_input   
            st.form_submit_button(current_translations["chat_save"]) 
...
if "messages" not in st.session_state:
    st.session_state["messages"] = [{"role": "assistant", "content": current_translations["chat_hello"]}]
...
if prompt := st.chat_input(current_translations["chat_imput_q"]):
    # Add to chat history
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")
    chat_preview = prompt[:50] + "..." if len(prompt) > 50 else prompt
    st.session_state.chat_history.insert(0, f"{timestamp}: {chat_preview}")
